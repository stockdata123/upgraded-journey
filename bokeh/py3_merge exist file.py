...
def getTodayRSI(stocksymbol, timespandays):
    ...
    todayRSI_list=[stocksymbol, todayprice, todayRSI]

    return todayRSI_list

def main():
    stocklist = ['CNI', 'PPL', 'PBA', 'BB', 'AAP', 'AMZN', 'MSFT', 'GOOG', 'FB', 'TD', 'ALXN', 
    'TSLA', 'GLD', 'BAC', 'SLV', 'NTDOY', 'REI', 'AAPL', 'FANG', 'BABA', 'OXY']
    #stocklist = ['AAPL', 'FANG'] #'MRU', 'IFC', 'LSPD', 'ELF', 'CJT']
    all_list = []
    ...
    df = pd.DataFrame(all_list, columns=['Symbol', 'ClosePrice', 'RSI'])
    print(df)
    df.to_csv(r'/home/lan/Documents/py3ds/output/dataWatch/datawatch1.csv')    
