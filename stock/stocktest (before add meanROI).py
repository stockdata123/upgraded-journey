...
def stockStdMethod(stockSymbol, stockCsvPath, savetofile):
    ...
    try:
        ...
        for i in range(1, len(df)):
            ...
            max(minstd0, stockinfo.get('std')),  stockinfo.get('stdw'), stockinfo.get('stdr'),  stockinfo.get('closeMean'), stockinfo.get('marketHigh'), stockinfo.get('marketLow'), stockinfo.get('gap')]
        ...
        dfrst.columns = ['stockSymbol', 'Date', 'marketPrice', 'std', 'stdw', 'stdr', 'mean', 'marketHigh', 'marketLow', 'gap']
        ...
        buy1Count = dfrst[dfrst['buy']<=-1].buy.count()
        sell1Count = dfrst[dfrst['sell']>=1].sell.count()
        buy2Count = dfrst[dfrst['buy']<=-2].buy.count()
        sell2Count = dfrst[dfrst['sell']>=2].sell.count()
        ...
        stdw = round(dfrst.head(1).stdw.values[0], 2)
        
        rptlst.append([stockSymbol, buy1Count, sell1Count, buy2Count, sell2Count, buy3Count, sell3Count, len(dfrst), marketPrice, gap, stdw])            
        ...
        if(savetofile):
            ...
            dfrst['meanstdrh'] = dfrst.apply(lambda x:  round((x['mean']+x['stdr']), 2), axis=1)
            ...
            dfrst['meannext'] = dfrst['mean'].shift(-1)
            """
            #to analyze accumulate energy
            dfrst['acc'] = dfrst.apply(lambda x: round((x['mean']/x['meannext']), 4), axis=1)
            dfrst['accnext'] = dfrst['acc'].shift(-1)
            dfrst['acccc'] = dfrst.apply(lambda x: round((x['acc']*x['accnext']), 4), axis=1)
            dfrst['meancc'] = dfrst.apply(lambda x: round((x['mean']*x['acccc']), 4), axis=1)
            """
        ...
        if(todaybuy<=-2):                
            rstpath = '/home/lan/Documents/py3ds/output/stockRpt/buy/'+ str(abs(todaybuy)) +stockSymbol+'_buy.csv'
            dfrst.to_csv(rstpath)
        if(todaysell>=2):
            rstpath = '/home/lan/Documents/py3ds/output/stockRpt/sell/'+ str(todaysell) + stockSymbol+'_sell.csv'
            dfrst.to_csv(rstpath)
...
if __name__ == "__main__":
    ...
    stockList = ['AAPL', 'ADYEN', 'AEM', 'AEZS', 'AGIO', 'ALGN', 'ALTR', 'ALXN', 'AMBA', 'AMD', 'AMGN', 'AMZN', 'APHA', 'APLS', 'APPN', 'APPS', 'ARKF', 'ARKG', 'ARKQ', 'ARKW', 'ARNA', 'ARWR', 'ATOM', 'ATVI', 'AUVI ', 'AVAV', 'BA', 'BABA', 'BAM', 'BBIO', 'BEAM', 'BGTC', 'BIDU', 'BIIB', 'BILI', 'BIP', 'BLMN', 'BLOK', 'BNTX', 'CDNA', 'CLNE', 'CNI', 'CNQ', 'CP', 'CSIQ', 'CRM', 'CRNC', 'CZR', 'DDD', 'DE', 'DKNG', 'EDIT', 'ERIC', 'ETSY', 'EXAS', 'EXPI', 'FANG', 'FATE', 'FB', 'FCEL', 'FLGT ', 'FLIR', 'FUV', 'GBTC', 'GLD', 'GLEN', 'GLXY', 'GOOG', 'GOOGL', 'GRWG', 'HD', 'HIMX', 'HIVE', 'HOME', 'HPQ', 'IBB', 'ICE', 'IDEX', 'ILMN', 'ILMN', 'IONS', 'IOVA', 'IRDM', 'IWM', 'IYR', 'JD', 'JG', 'JPM', 'KL', 'KOD', 'KOPN ', 'KTOS', 'LLY', 'MA', 'MARA', 'MGNI', 'MOOV', 'MRK', 'MRNA', 'MSFT', 'MSTR', 'MTLS', 'MTSL', 'MVIS', 'NVAX', 'NVDA', 'NVTA', 'PACB', 'PDD', 'PENN', 'PERI', 'PFE', 'PINS', 'PLAY', 'PLL', 'PSTG', 'PTC', 'PYPL', 'QCOM', 'QQQ', 'QTRX', 'RDFN', 'REKR', 'RIDE', 'RIO', 'RIOT', 'ROKU', 'SAGE', 'SGEN', 'SHOP', 'SI', 'SIVB', 'SLV', 'SNAP', 'SONO', 'SPOT', 'SPY', 'SQ', 'SSYS', 'STAA', 'STMN', 'STNE', 'TCEHY', 'TD', 'TDOC', 'TGTX', 'TIGR', 'TLRY', 'TLT', 'TNA', 'TQQQ', 'TRIL', 'TRMB', 'TSLA', 'TTD', 'TTOO', 'TWST', 'TXN', 'V', 'VCYT', 'VERU', 'WKHS', 'WMT', 'WWR', 'XBI', 'XHB', 'XLE', 'Z', 'ZG', 'ZS']
    ...
    dfrpt.columns = ['stockSymbol', 'buy1std', 'sell1std', 'buy2std', 'sell2std', 'buy3std', 'sell3std', 'sampleSize', 'marketPrice', 'gap', 'stdw']
