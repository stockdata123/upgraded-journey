...
def getStockInfo(dfseleced):
    ...
    try:
        ...
        pastCloseStd1week = round(stockPrice1week.Close.std(), 2)
...
def stockStdMethod(stockSymbol, stockCsvPath, savetofile):
    ...
    try:
        ...
        for i in range(1, len(df)):
            ...
            max(minstd0, stockinfo.get('std')),  stockinfo.get('stdr'),  stockinfo.get('closeMean'), stockinfo.get('marketHigh'), stockinfo.get('marketLow')]
        ...
        for buya in drange(0.1, 1.5, 0.1):
            ...
            for sella in drange(1.0, 3.0, 0.2):
                ...
                dfrst.columns = ['stockSymbol', 'Date', 'marketPrice', 'std', 'stdr', 'mean', 'marketHigh', 'marketLow']
        ...
        if(savetofile):
            ...
            if((meanPrice0-marketLow0 - 1*std0)>0.5):
                ...
                rstpath = '/home/lan/Documents/py3ds/output/stockRpt/buy/'+ stockSymbol+'_buy.csv'
                dfrst.to_csv(rstpath)
            if((marketHigh0-meanPrice0-2*std0)>0.5):
                dfrst['buy'] = dfrst.apply(lambda x: -1 if (((x['mean']- x['marketLow'])>0.5)) else 0, axis=1)
                dfrst['sell'] = dfrst.apply(lambda x: 1 if (((x['marketHigh'] - x['mean'] - x['std']) >0.5)) else 0, axis=1)
                rstpath = '/home/lan/Documents/py3ds/output/stockRpt/sell/'+ stockSymbol+'_sell.csv'
                dfrst.to_csv(rstpath)
